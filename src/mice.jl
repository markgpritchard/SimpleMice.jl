...
function selectinitialvalues(v::Vector{<:Union{Missing, T}}, N) where T
    ...
    return ImputedData{N, T}[ ismissing(a) ? selectinitialvalue(nonmissingv, N) : ImputedNonMissingData{N, T}(a) for a ∈ v ]
end

selectinitialvalue(nonmissingv, N) = ImputedMissingData(MVector{N}([ sample(nonmissingv) for _ ∈ 1:N ]))

function imputevalues!(df, vars, N, M)
    for i ∈ 1:N 
        for _ ∈ 1:M
            for v ∈ vars 
                ...
                imputevalues!(getproperty(df, v), newvalues, i)
            end
        end
    end
end

function imputevalues!(dfvector, newvalues, i)
    for j ∈ eachindex(dfvector) 
        imputevalue!(dfvector, dfvector[j], newvalues[j], i, j)
    end
end

imputevalue!(dfvector, dfvalue::ImputedNonMissingData, newvalue, i, j) = nothing 

imputevalue!(dfvector, dfvalue::ImputedMissingData, newvalue, i, j) = dfvector[j].v[i] = newvalue
